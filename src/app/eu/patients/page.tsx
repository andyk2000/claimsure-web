...
export default function PatientsPage() {
  ...
  const handleViewPatient = (id: number) => {
    router.push(`/eu/patients/${id}`);
  };

  const handleViewMedicalHistory = (id: number) => {
    router.push(`/eu/patients/${id}/medical-history`);
  };

  const handleNewRequest = (id: number) => {
    router.push(`/eu/dashboard/Requests/new-request?patientId=${id}`);
  };
              ...
              <tr key={patient.id}>
                ...
                <td>
                  <div className={styles.actionButtons}>
                    ...
                    <button 
                      className={styles.actionButton}
                      onClick={() => handleNewRequest(patient.id)}
                      title="Create New Request"
                    >
                      <Icon icon="ph:note-pencil" width="18" height="18" />
                    </button>
                  </div>
                </td>
              </tr>
